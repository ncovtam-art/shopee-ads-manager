...
import { DollarSign, TrendingUp, Zap, Star, BarChart3, Package, ShoppingBag, ArrowUpRight, ArrowDownRight, Upload, Megaphone, AlertTriangle } from "lucide-react";
  ...
  const fetchAll = async () => {
    ...
    if (pageData) {
      const rows: PageRow[] = pageData.map((p: any) => {
        const adSpend = Number(p.total_ad_spend);
        const commission = Number(p.total_commission);
        const profit = commission - adSpend;
        return {
          name: p.page_name, adSpend, commission, profit,
          roi: adSpend > 0 ? Math.round((profit / adSpend) * 1000) / 10 : null,
    ...
    return (
      <div>
        ...
        <div className="glass-card rounded-2xl p-12 text-center border border-[var(--border)]">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#ee4d2d]/10 to-[#ff6b47]/5 flex items-center justify-center mx-auto mb-5 ring-1 ring-[#ee4d2d]/10"><Upload size={36} className="text-[var(--accent)]" /></div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-6 gap-2.5 mb-4">
        {kpis.map((m, i) => (
          <div key={i} className={`glass-card rounded-xl p-3.5 border border-[var(--border)] ${m.glow} transition-all hover:scale-[1.02]`}>
            <div className="flex items-center gap-1.5 mb-2"><div className="w-6 h-6 rounded-md flex items-center justify-center" style={{ background: `${m.color}12` }}><m.icon size={12} style={{ color: m.color }} /></div><span className="text-[10px] text-[var(--muted-foreground)] font-medium">{m.label}</span></div>
      ...
      <div className="grid grid-cols-3 gap-2.5 mb-5">
        <div className="glass-card rounded-xl p-3 text-center border border-[var(--border)]"><div className="font-mono text-2xl font-bold">{pageRows.length}</div><div className="text-[10px] text-[var(--muted-foreground)] mt-0.5">Page</div></div>
        <div className="glass-card rounded-xl p-3 text-center border border-green-500/10 glow-green"><div className="font-mono text-2xl font-bold text-green-400 flex items-center justify-center gap-1"><ArrowUpRight size={16} />{profitPages}</div><div className="text-[10px] text-[var(--muted-foreground)] mt-0.5">Page lãi</div></div>
        <div className="glass-card rounded-xl p-3 text-center border border-red-500/10 glow-red"><div className="font-mono text-2xl font-bold text-red-400 flex items-center justify-center gap-1"><ArrowDownRight size={16} />{lossPages}</div><div className="text-[10px] text-[var(--muted-foreground)] mt-0.5">Page lỗ</div></div>
      </div>

      {pageRows.length > 0 && (
        <div className="glass-card rounded-2xl border border-[var(--border)] mb-4 overflow-hidden">
          <div className="px-5 py-3.5 border-b border-[var(--border)] flex items-center justify-between"><div><span className="text-sm font-semibold">Top Pages</span><span className="text-[10px] text-[var(--muted-foreground)] ml-2">theo lợi nhuận</span></div><Link href="/reports" className="text-[11px] text-[var(--accent)] hover:underline flex items-center gap-0.5 font-medium">Xem P&L <ArrowUpRight size={10} /></Link></div>
          <div className="p-4 h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={pageRows} layout="vertical" margin={{ left: 0, right: 20 }}>
                ...
                <Bar dataKey="profit" name="Lợi nhuận" radius={[0, 4, 4, 0]} maxBarSize={24}>
                  {pageRows.map((e, i) => (<Cell key={i} fill={e.profit >= 0 ? "#22c55e" : "#ef4444"} fillOpacity={0.85} />))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[650px]">
              <thead><tr className="border-b border-[var(--border)]">
                ...
                <th className="text-right px-4 py-2.5 text-[10px] font-semibold text-[var(--muted-foreground)] uppercase tracking-wider">Đơn</th>
              </tr></thead>
              <tbody>
                {pageRows.map((c, i) => (
                  <tr key={i} className="row-hover border-b border-[var(--border)] transition-colors">
                    <td className="px-4 py-2.5"><div className="flex items-center gap-2.5"><div className={`w-7 h-7 rounded-lg flex items-center justify-center text-[10px] font-bold ${i < 3 ? "bg-gradient-to-br from-amber-500 to-amber-600 text-white" : c.profit > 0 ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"}`}>{i < 3 ? ["🥇","🥈","🥉"][i] : i+1}</div><span className="text-xs font-medium">{c.name}</span></div></td>
                    <td className="px-4 py-2.5 text-right font-mono text-xs text-red-400">{c.adSpend > 0 ? formatCompact(c.adSpend) : "—"}</td>
                    <td className="px-4 py-2.5 text-right font-mono text-xs text-indigo-400">{c.commission > 0 ? formatCompact(c.commission) : "—"}</td>
                    ...
                    <td className="px-4 py-2.5 text-right font-mono text-xs text-[var(--muted-foreground)]">{c.orders}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <div className="px-5 py-2.5 border-t border-[var(--border)] text-center"><Link href="/pages" className="text-[11px] text-[var(--accent)] hover:underline font-medium">Xem chi tiết từng Page →</Link></div>
        </div>
      )}

      {lossPages > 0 && (
        <div className="glass-card rounded-2xl border border-red-500/10 glow-red mt-4 overflow-hidden">
          <div className="px-5 py-3 border-b border-red-500/10 flex items-center gap-2"><AlertTriangle size={14} className="text-red-400" /><span className="text-sm font-semibold text-red-400">Cảnh báo: {lossPages} page đang lỗ</span></div>
          {pageRows.filter(c => c.profit < 0).map((c, i) => (
            <div key={i} className="px-5 py-2.5 flex items-center justify-between border-b border-[var(--border)] last:border-0 row-hover"><div className="flex items-center gap-2"><span className="w-1.5 h-1.5 rounded-full bg-red-400 pulse-dot" /><span className="text-xs font-medium">{c.name}</span></div><span className="font-mono text-xs text-red-400 font-bold">{formatCompact(c.profit)}</span></div>
          ))}
        </div>
      )}
    </div>
