export type Period = "today" | "yesterday" | "7d" | "30d" | "this_month" | "last_month" | "all";

export function getDateRange(period: Period): { from: string | null; to: string | null; label: string } {
  const now = new Date();
